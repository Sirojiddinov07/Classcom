...
class UserModeratorSerializer(serializers.ModelSerializer):
    ...
    def validate_phone(self, value):
        if models.User.objects.filter(phone=value, validated_at__isnull=False).exists():
...
class ModeratorCreateSerializer(serializers.ModelSerializer):
    ...
    region = serializers.PrimaryKeyRelatedField(queryset=Region.objects.all(), source="user.region")
    district = serializers.PrimaryKeyRelatedField(queryset=District.objects.all(), source="user.district")
