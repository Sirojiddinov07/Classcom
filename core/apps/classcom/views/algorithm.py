...
from core.apps.classcom.models import Weeks, Plan, Schedule, ScheduleChoices
    ...
    def get(self, request):
        ...
        try:
            ...
            plan = Plan.objects.filter(
                science=science, classes=classes
            ).first()
            if not plan:
                return Response(
            ...
            topics = plan.topic.filter(weeks=week_count).order_by(
                "sequence_number"
            )
